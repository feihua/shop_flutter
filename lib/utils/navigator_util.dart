...
class NavigatorUtil {
  ...
  static goShopMainPage(BuildContext context) {
    Application.router?.navigateTo(context, Routers.home, transition: TransitionType.inFromRight, replace: true);
  }
  ...
  static goCategoryGoodsListPage(BuildContext context, String categoryName, int categoryId) {
    ...
    Application.router?.navigateTo(
        context, Routers.categoryGoodsList + "?categoryName=$categoryNameText&categoryId=$categoryId",
        transition: TransitionType.inFromRight);
  }

  //跳转至注册页面
  static goRegister(BuildContext context) {
    Application.router?.navigateTo(context, Routers.register, transition: TransitionType.inFromRight);
  }
  ...
  static goGoodsDetails(BuildContext context, int goodsId) {
    //传递参数为商品Id
    Application.router
        ?.navigateTo(context, Routers.goodsDetail + "?goodsId=$goodsId", transition: TransitionType.inFromRight);
  }
  ...
  static goFillInOrder(BuildContext context, int cartId) {
    //参数为购物车Id
    Application.router
        ?.navigateTo(context, Routers.fillInOrder + "?cartId=$cartId", transition: TransitionType.inFromRight);
  }

  //跳转至地址页面
  static Future? goAddress(BuildContext context) {
    return Application.router?.navigateTo(context, Routers.address, transition: TransitionType.inFromRight);
  }
  ...
  static Future? goAddressEdit(BuildContext context, int addressId) {
    ...
    return Application.router
        ?.navigateTo(context, Routers.addressEdit + "?addressId=$addressId", transition: TransitionType.inFromRight);
  }

  //跳转至收藏页面
  static goCollect(BuildContext context) {
    Application.router?.navigateTo(context, Routers.mineCollect, transition: TransitionType.inFromRight);
  }

  //跳转至关于我们页面
  static goAboutUs(BuildContext context) {
    Application.router?.navigateTo(context, Routers.aboutUs, transition: TransitionType.inFromRight);
  }
  ...
  static Future? goOrderDetail(BuildContext context, int orderId, String token) {
    //参数有订单Id及token
    return Application.router?.navigateTo(context, Routers.mineOrderDetail + "?orderId=$orderId&token=$token",
        transition: TransitionType.inFromRight);
  }
  ...
  static goWebView(BuildContext context, String title, String url) {
    ...
    Application.router?.navigateTo(context, Routers.webView + "?title=$titleName&&url=$urlEncode",
        transition: TransitionType.inFromRight);
  }
  ...
  static goBrandDetail(BuildContext context, String titleName, int id) {
    ...
    Application.router
        ?.navigateTo(context, Routers.brandDetail + "?titleName=$title&id=$id", transition: TransitionType.inFromRight);
  }
}
